...
st.markdown("Preencha as informações abaixo para gerar automaticamente o contrato personalizado.")

# Formulário de entrada
dados = {}
dados["{{CONTRATANTE_NOME}}"] = st.text_input("Nome do Contratante")
dados["{{CPF}}"] = st.text_input("CPF")
dados["{{RG}}"] = st.text_input("RG")
dados["{{EMAIL}}"] = st.text_input("Email")
dados["{{ENDERECO}}"] = st.text_area("Endereço completo")
dados["{{OBJETO}}"] = st.text_area("Objeto do Contrato")
...
parcelas = []
num_parcelas = st.number_input("Número de parcelas", min_value=1, max_value=36, value=1)

for i in range(num_parcelas):
    valor = st.number_input(f"Valor da {i+1}ª parcela", min_value=0.0, step=0.01, key=f"valor_{i}")
